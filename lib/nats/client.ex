...
defmodule Nats.Client do
  ...
  def init(orig_opts) do
    ...
    case Connection.start_link(parent, opts) do
      ...
      other ->
        {:error, "unable to start connection link", other}
    end
  end

  def handle_info(
        {:msg, subject, sid, reply, what},
        state = %{subs_by_sid: subs_by_sid, status: client_status}
      )
  ...
  def terminate(_reason, state = %{status: status}) when status != :closed do
  ...
  def handle_call(
        {:unsub, ref = {sid, who}, afterReceiving},
        _from,
        state = %{subs_by_sid: subs_by_sid, subs_by_pid: subs_by_pid}
      ) do
    case Map.get(subs_by_sid, sid, nil) do
      ^who ->
        ...
        new_subs_by_pid =
          case Map.size(other_subs_for_pid) > 0 do
  ...
  def handle_call(
        {:sub, who, subject, queue},
        _from,
        state = %{subs_by_sid: subs_by_sid, subs_by_pid: subs_by_pid, next_sid: next_sid}
      ) do
    ...
    state = %{state | subs_by_sid: subs_by_sid, subs_by_pid: subs_by_pid, next_sid: next_sid + 1}
    send_cmd(state, {:sub, subject, queue, sid})
    # IO.puts "subscribed!! #{inspect(state)}"
    {:reply, {:ok, {sid, who}}, state}
  end

  def handle_call({:cmd, encoded, flush?}, from, state = %{status: client_status})
      when client_status != :closed do
  ...
  def pub(self, subject, what) do
    pub(self, subject, nil, what)
  end

  def pub(self, subject, reply, what),
    do: GenServer.call(self, {:cmd, Nats.Parser.encode({:pub, subject, reply, what}), false})
