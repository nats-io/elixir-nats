...
defmodule Nats.ClientTest do
  ...
  test "Open a default client" do
    ...
    :ok = Client.flush(con)
    :ok = GenServer.call(con, {:cmd, Nats.Parser.encode({:ok}), false})
    :ok = Client.flush(con)
    # get coverage...
    :ok = GenServer.call(con, {:cmd, Nats.Parser.encode({:ping}), false})
    :ok = Client.flush(con)
  ...
  test "Client vs. server auth" do
    ...
    # should succeed
    opts = %{port: TestHelper.auth_port(), auth: %{user: "user", pass: "pass"}}
    {:ok, conn} = Client.start(opts)
    Client.stop(conn)
    ...
    opts = %{port: TestHelper.auth_port(), auth: %{user: "oops", pass: "oops22"}}
